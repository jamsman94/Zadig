...
func ListUserGroups(searchName string, pageNum, pageSize int, logger *zap.SugaredLogger) ([]*UserGroupResp, int64, error) {
	resp := make([]*UserGroupResp, 0)
	tx := repository.DB.Begin()

	groups, count, err := orm.ListUserGroups(searchName, pageNum, pageSize, tx)
	if err != nil {
		tx.Rollback()
