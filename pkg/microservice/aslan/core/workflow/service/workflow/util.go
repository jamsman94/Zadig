...
func renderScriptedVariableOptions(script, callFunction string, userInput map[string]string) ([]string, error) {
	// first render callFunc with the special Variables
	parsedCallfunc := parseCallFuncWithSpecialVariables(callFunction, userInput)

	t, err := template.New("scriptRender").Parse(parsedCallfunc)
	if err != nil {
		return nil, err
	}
